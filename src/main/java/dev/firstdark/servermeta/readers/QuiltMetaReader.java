...
import dev.firstdark.servermeta.database.tables.QuiltVersionsTable;
...
/**
 * @author HypherionSA
 * Worker to pull quilt versions and build the database for it
 */
public class QuiltMetaReader extends Thread {

    public QuiltMetaReader() {
        this.setName("Quilt Meta Updater");
    }

    @Override
    public void run() {
        MetaServer.LOGGER.info("Start Refreshing Quilt Meta");
        try {
            ...
            JsonArray fabricLoader = gson.fromJson(getJson("https://meta.quiltmc.org/v3/versions/loader"), JsonArray.class);

            manifest.mVersions.forEach(v -> {
                if (v.mId.startsWith("3D"))
                    return;

                DefaultArtifactVersion lowest = new DefaultArtifactVersion("1.18");
                ...
                fabricLoader.asList().forEach(fv -> {
                    ...
                    if (DatabaseController.INSTANCE.getDB().findById(fabricV, QuiltVersionsTable.class) == null) {
                        QuiltVersionsTable table = new QuiltVersionsTable();
                        table.setVersion(fabricV);
                        table.setHash("");
                        table.setDownloadUrl(String.format("https://maven.quiltmc.org/repository/release/org/quiltmc/quilt-loader/%s/", rawV));
                        table.setSize(0);
            ...
            MetaServer.LOGGER.error("Failed to read quilt meta", e);
        }
    ...
    public static boolean isLowerThan(String version) {
        ...
        return Integer.parseInt(String.format("%s%s", majorVersion, minorVersion)) < 2137;
    }
}
